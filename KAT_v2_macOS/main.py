...
from PySide6.QtGui import QPixmap, QFont, QPalette, QColor, QImage
...
def safe_open_file_dialog(parent, caption="Open File", file_filter="CSV Files (*.csv);;All Files (*)"):
    try:
        ...
        options |= QFileDialog.DontUseNativeDialog  # Important for macOS apps
...
class KineticAnalysisTool(QMainWindow):
    ...
    #def select_file(self):
    #    path = safe_open_file_dialog(self)
    #    if path:
    #        self.file_path_input.setText(path)
    #        try:
    #            with open("path_data.txt", "w") as f:
    #                f.write(f"{path}\n")
    #        except Exception as e:
    #            QMessageBox.warning(self, "File Save Error", f"Could not write to path_data.txt:\n{str(e)}")
    #    else:
    #        QMessageBox.warning(self, 'Missing File', 'No file selected. Please choose or enter a valid CSV file.')
